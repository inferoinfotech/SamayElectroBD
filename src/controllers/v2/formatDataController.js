...
// Helper function to reliably parse any timestamp or number into { monthIndex, year }
const parseDateToMonthYear = (dateVal) => {
...
// Helper function to verify if a date value matches the target month and year
const doesDateValueMatch = (dateVal, targetMonth, targetYear) => {
  const parsed = parseDateToMonthYear(dateVal);
  ...
  const targetMonthIndex = monthNames.indexOf(targetMonth) + 1;

  return parsed.monthIndex === targetMonthIndex && parsed.year === parseInt(targetYear);
};
...
const validateExcelFile = (filePath, originalName = '', selectedMonth, selectedYear) => {
  ...
  // Check if data matches selected month & year
  let hasSelectedPeriodData = false;
  if (detectedFormat === 'F1' || detectedFormat === 'F2') {
    for (let i = 1; i < jsonData.length; i++) {
      const row = jsonData[i];
      if (row && row[4] && doesDateValueMatch(row[4], selectedMonth, selectedYear)) {
        hasSelectedPeriodData = true;
        break;
      }
    }
  } else if (detectedFormat === 'F3') {
    for (let i = 18; i < jsonData.length; i++) {
      const row = jsonData[i];
      if (row && row[0] && doesDateValueMatch(row[0], selectedMonth, selectedYear)) {
        hasSelectedPeriodData = true;
        break;
      }
    }
  } else if (detectedFormat === 'F4') {
    for (let i = 3; i < jsonData.length; i++) {
      const row = jsonData[i];
      if (row && row[1] && doesDateValueMatch(row[1], selectedMonth, selectedYear)) {
        hasSelectedPeriodData = true;
        break;
      }
    }
  } else if (detectedFormat === 'F5') {
    for (let i = 8; i < jsonData.length; i++) {
      const row = jsonData[i];
      if (row && row[0] && doesDateValueMatch(row[0], selectedMonth, selectedYear)) {
        hasSelectedPeriodData = true;
  ...
  if (!hasSelectedPeriodData) {
    return {
      valid: false,
      message: `Selected month/year data (${selectedMonth} ${selectedYear}) not found in this file`,
...
const processExcel = async (filePath, month, year, formatType) => {
    ...
    if (!f1Sheet) {
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        if (row && row.length > 0) {
          ...
          if (row[4] && !doesDateValueMatch(row[4], month, year)) {
            continue;
          }
      ...
      if (!hasData && formatType === 'F1') throw new Error("F1 sheet has no data matching the selected period.");
  ...
  if (formatType === 'All' || formatType === 'F2') {
    ...
    if (!f2Sheet) {
      ...
      if (!hasData && formatType === 'F2') throw new Error("F2 sheet has no data matching the selected period.");
      ...
      if (meterSerialNumber) {
        ...
        for (let i = 18; i < jsonData.length; i++) {
          ...
          if (row && row.length > 0 && row[0]) {
            // Filter by selected month & year
            if (!doesDateValueMatch(row[0], month, year)) {
              continue;
            }
        ...
        if (dataEntries.length === 0 && formatType === 'F3') throw new Error("F3 sheet has no data matching the selected period.");
    ...
    if (!f4Sheet) {
      ...
      for (let i = 3; i < jsonData.length; i++) {
        ...
        if (row && row.length > 1 && row[1]) {
          // Filter by selected month & year
          if (!doesDateValueMatch(row[1], month, year)) {
            continue;
          }
      ...
      if (dataEntries.length === 0 && formatType === 'F4') throw new Error("F4 sheet has no data matching the selected period.");
  ...
  if (formatType === 'All' || formatType === 'F5') {
    ...
    if (!f5Sheet) {
      ...
      const meterSerialNumber = 'F5_METER';
      ...
      if (dataEntries.length === 0 && formatType === 'F5') throw new Error("F5 sheet has no data matching the selected period.");
  ...
  if (formatType === 'F5') {
    ...
    csvData = allIntervals.map(interval => {
      ...
      return {
        ...
        'KW Imp.': entry ? (entry.kwImp || 0) : 0,
        'KW Exp.': entry ? (entry.kwExp || 0) : 0,
        'KVA Imp.': entry ? (entry.kvaImp || 0) : 0,
        'KVA Exp.': entry ? (entry.kvaExp || 0) : 0,
        'KWh Imp. [1.0.1.29.0.255]': entry ? (entry.kwhImp || 0) : 0,
        'KWh Exp. [1.0.2.29.0.255]': entry ? (entry.kwhExp || 0) : 0,
        'Net KWh [1.0.16.29.0.255]': entry ? (entry.netKwh || 0) : 0,
        'KVAh Imp. [1.0.9.29.0.255]': entry ? (entry.kvahImp || 0) : 0,
        'KVAh Exp. [1.0.10.29.0.255]': entry ? (entry.kvahExp || 0) : 0,
        'Kvarh Lg Dur Wh Imp. [1.0.5.29.0.255]': entry ? (entry.kvarhLgDurWhImp || 0) : 0,
        'Kvarh Ld Dur Wh Imp. [1.0.8.29.0.255]': entry ? (entry.kvarhLdDurWhImp || 0) : 0,
        'Kvarh Lg Dur Wh Exp. [1.0.7.29.0.255]': entry ? (entry.kvarhLgDurWhExp || 0) : 0,
        'Kvarh Ld Dur Wh Exp. [1.0.6.29.0.255]': entry ? (entry.kvarhLdDurWhExp || 0) : 0,
        'Avg. R-Ph  Volt [1.0.32.27.0.255]': entry ? (entry.avgRPhVolt || 0) : 0,
        'Avg. Y-Ph  Volt [1.0.52.27.0.255]': entry ? (entry.avgYPhVolt || 0) : 0,
        'Avg. B-Ph  Volt [1.0.72.27.0.255]': entry ? (entry.avgBPhVolt || 0) : 0,
        'Average Volt': entry ? (entry.averageVolt || 0) : 0,
        'Avg. R-Ph  Amp. [1.0.31.27.0.255]': entry ? (entry.avgRPhAmp || 0) : 0,
        'Avg. Y-Ph  Amp. [1.0.51.27.0.255]': entry ? (entry.avgYPhAmp || 0) : 0,
        'Avg. B-Ph  Amp. [1.0.71.27.0.255]': entry ? (entry.avgBPhAmp || 0) : 0,
        'Average Amp.': entry ? (entry.averageAmp || 0) : 0,
        'Coded Freq. [1.0.207.29.0.255]': entry ? (entry.codedFreq || '') : '',
        'Avg. Freq. [1.0.14.27.0.255](Hz.)': entry ? (entry.avgFreq || 0) : 0,
        'power off minutes of last IP [1.0.139.29.0.255] (Min)': entry ? (entry.powerOffMinutes || 0) : 0,
        'Kvarh High Import [1.0.146.29.0.255]': entry ? (entry.kvarhHighImport || 0) : 0,
        'Kvarh High Export [1.0.147.29.0.255]': entry ? (entry.kvarhHighExport || 0) : 0,
        'Kvarh Low Import [1.0.148.29.0.255]': entry ? (entry.kvarhLowImport || 0) : 0,
        'Kvarh Low Export [1.0.149.29.0.255]': entry ? (entry.kvarhLowExport || 0) : 0,
        'Kvarh Net Reac.High(Q1 + Q2 - Q3 - Q4)[1.0.196.8.0.255]': entry ? (entry.kvarhNetReacHigh || 0) : 0,
        'Kvarh Net Reac.Low(Q1 + Q2 - Q3 - Q4)[1.0.197.8.0.255]': entry ? (entry.kvarhNetReacLow || 0) : 0,
        'Power Factor Imp.': entry ? (entry.powerFactorImp || '') : '',
        'Power Factor Exp.': entry ? (entry.powerFactorExp || 0) : 0
      };
    });
  } else {
    ...
    if (formatType === 'F1') {
      ...
      if (data.dataEntries && data.dataEntries.length > 0) {
        data.dataEntries.forEach((entry) => {
          if (entry.time) {
            const match = entry.time.match(/(\d{2})\/(\d{2})\/(\d{4})\s+(\d{1,2}):(\d{2})/);
            if (match) {
              let [, day, monthNum, yearNum, hours, minutes] = match;
              day = parseInt(day);
              monthNum = parseInt(monthNum);
...
exports.validateBulkFiles = (req, res) => {
  uploadMany(req, res, async (err) => {
    ...
    if (!month || !year) {
      ...
      return res.status(400).json({ success: false, message: 'Please select Month and Year first' });
    }
    ...
    const results = req.files.map((f) => {
      const validation = validateExcelFile(f.path, f.originalname, month, year);
      if (fs.existsSync(f.path)) fs.unlinkSync(f.path);
      return {
        file: f.originalname,
        ...validation,
