...
exports.deleteMainClient = async (req, res) => {
  try {
    ...
    // NEW CODE FOR CLIENT PROGRESS UPDATES
    const progressUpdate = await ClientProgress.updateMany(
      { "clients.clientId": clientId },
      { $pull: { clients: { clientId: clientId } } }
    );
    logger.info(`Removed client from ${progressUpdate.modifiedCount} Client Progress monthly reports`);

    const progressFieldUpdate = await ClientProgressField.updateMany(
      { "clients.clientId": clientId },
      { $pull: { clients: { clientId: clientId } } }
    );
    logger.info(`Removed client from ${progressFieldUpdate.modifiedCount} Client Progress Field templates`);

    // Also delete any ClientProgressField documents that are now empty
    await ClientProgressField.deleteMany({ clients: { $size: 0 } });

    logger.info(`Deleted Client Progress Field entries for Main Client: ${clientId}`);
